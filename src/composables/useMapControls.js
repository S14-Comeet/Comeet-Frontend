...
export function useMapControls(map) {
  ...
  const controlsBottomStyle = computed(() => {
    ...
    return { bottom: bottomValues[sheetState.value] || "calc(100px + 16px)" };
  });
  ...
  const getZoomCenter = () => {
    ...
    if (focusedLocation.value) {
      return new naver.maps.LatLng(
        focusedLocation.value.lat,
        focusedLocation.value.lng
      );
    }
    ...
    const visualCenterOffset = new naver.maps.Point(
      centerOffset.x,
